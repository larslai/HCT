...
function updatePolygonArea(spider_web_id,spider_web_option_id){
	var spider_web_point = [];
	$("#"+spider_web_option_id+' > div ').each(function (index, element) {
		var value = parseFloat($( element ).find('label').html());
		spider_web_point.push(value);
                });
	var chart = $('#'+spider_web_id).highcharts();
	chart.series[0].setData(spider_web_point);
	//alert(spider_web_point);
	//updateSpiderWeb( spider_web_id,spider_web_point);
	/*
	$('#'+spider_web_option_id+' > div > span').each(function() {
...
function updateSpiderWeb(spider_web_id ,radar_data){
	var chart = $('#'+spider_web_id).highcharts();
	//alert(radar_data);
	//var radar_data=[radar_data];
	chart.series[0].setData(radar_data);
	//

	//spider_web('spider_web','aa','bb',radar_data);
}